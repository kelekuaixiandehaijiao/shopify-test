    ...
    const fbCustomerShop = {
        ...
        getSoldOut: function () {
            if(window.location.pathname.indexOf('boom-bike')> -1){
              ...
              } else if(Shopify &&  Shopify.country && (Shopify.country.toUpperCase() == 'GB') || window.location.pathname.indexOf('en-') > -1){
        ...
        getDeliverCode: function () {
            ...
            if($('.js-select-countrycode').length) {
                let bikeArr=[
                    'boom-bike-arctic-white','boom-bike-mist-grey','boom-bike-tuscany-yellow','lit-bike-moonbow-beige',
                    'lit-bike-snowpeak-white','lit-bike','lit-bike-klarna-limited-edition'
                ]
                let productColor=$(`.js-fbchoose-productcolor[data-type='${this.bikeType}']`)
                var ulElement = document.querySelector(".ebike-choose-ul");
                // 检查子元素的数量
                var elementCount = ulElement.children.length;
                let cashow = "true"
                let gbshow = "true"
                let aushow = "true"
                console.log(window.location.href.includes(bikeArr[1]),'ssss')
                if(window.location.href.includes(bikeArr[1])||window.location.href.includes(bikeArr[2])){
                    cashow="false"
                }
                if(elementCount>2||elementCount==1){
                    cashow="true"
                }
                if(window.location.href.includes(bikeArr[4])||window.location.href.includes(bikeArr[6])){
                    gbshow="false"
                    aushow="false"
                }
                if(elementCount==2||elementCount==1){
